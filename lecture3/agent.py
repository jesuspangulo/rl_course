...
class MonteCarlo():
    ...
    def _update_q(self):
        states_actions = []
        [states_actions.append((state, action)) for state,action,_ in self.episode if (state, action) not in states_actions]
        for state, action in states_actions:
            first_occurence = next(i for i,step in enumerate(self.episode) if step[0] == state and step[1] == action)
            G = sum([step[2]*(self.gamma**i) for i,step in enumerate(self.episode[first_occurence:])])  
            self.returns[state][action] += G
            self.returns_n[state][action] += 1
            self.q[state][action] = self.returns[state][action] / self.returns_n[state][action]
